...
import { setupSurveyInteractions, showResultsTab, updateProgress } from './survey.js';
...
['exportData','showHeatmapDetail','filterTasks','pushToDashboard'].forEach(fn => {
...
function wireSubmission() {
  ...
  form.addEventListener('submit', async e => {
    ...
    const aspects = Array.from(document.querySelectorAll('.aspect-btn.selected')).map(
      b => b.dataset.aspect,
    );
    const payload = {
...
function displayResults(data) {
  ...
  if (overallEl) overallEl.textContent = (composite_index * 100).toFixed(0) + '';
  ...
  try {
    ...
    if (qBlock && data.quality_score !== undefined) {
      ...
      let cls = '';
      if (score < 0.55) cls = 'risk'; else if (score < 0.75) cls = 'warn';
      if (badge) {
        ...
        const icon = cls === 'risk' ? '#icon-risk' : cls === 'warn' ? '#icon-warn' : '#icon-check';
        ...
        const tipParts = [];
        if (typeof q.diversity === 'number') tipParts.push(`Diversity ${(q.diversity * 100).toFixed(0)}%`);
        if (typeof q.longest_run === 'number') tipParts.push(`Longest Run ${q.longest_run}`);
        if (typeof q.non_lexical_ratio === 'number') tipParts.push(`Non-lex ${(q.non_lexical_ratio * 100).toFixed(0)}%`);
        if (typeof q.gibberish_score === 'number') tipParts.push(`Gibberish ${(q.gibberish_score * 100).toFixed(0)}%`);
        if (typeof q.common_word_ratio === 'number') tipParts.push(`Common ${(q.common_word_ratio * 100).toFixed(0)}%`);
        if (typeof q.entropy === 'number') tipParts.push(`Entropy ${q.entropy.toFixed(2)}`);
      ...
      if (elig) {
        ...
        elig.textContent = eligible ? 'Eligible for eGift' : 'Ineligible (low-effort detected)';
...
function wireMisc() {
  qs('invite-trigger')?.addEventListener('click', () => openInvite(window.CANDIDATE_TOKEN));
  qs('open-hud')?.addEventListener('click', () => toggleHud());
  // Autofill helpers
  qs('autofill')?.addEventListener('click', () => {
    qs('well').value = 'Supportive interviewer detailed role growth values alignment culture';
    qs('better').value =
      'Faster feedback clarity next steps comp range fewer rounds timeline expectations';
    ['well', 'better'].forEach(id => document.getElementById(id).dispatchEvent(new Event('input')));
  });
