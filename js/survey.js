...
export function setupSurveyInteractions() {
  ...
  document
    .getElementById('well')
    ?.addEventListener('input', () => enforceExactWords('well', 'well-count', 15));
  document
    .getElementById('better')
    ?.addEventListener('input', () => enforceExactWords('better', 'better-count', 15));
...
export function showResultsTab(tabName) {
  ...
  const match = tabsBtns.find(b => (b.textContent || '').toLowerCase().includes(tabName));
  if (match) {
