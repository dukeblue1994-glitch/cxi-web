...
// Scheduled variant uses the same logic as HTTP dlq-retry, inlined to avoid module import issues in some environments
export default async () => {
  try {
    ...
    return new Response(
      JSON.stringify({ ok: true, processed, scheduled: true }),
      {
        status: 200,
        headers: {
