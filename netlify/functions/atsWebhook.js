...
async function greenhouseAdapter(data) {
  ...
  if (!apiKey || !applicationId) return { forwarded: false, reason: 'missing_env' };
  const note = {
...
async function leverAdapter(data) {
  ...
  if (!apiKey || !opportunityId) return { forwarded: false, reason: 'missing_env' };
  const note = {
...
export default async request => {
  try {
    ...
    const event_id = body.id || `evt_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const idempotency_key = request.headers.get('x-idempotency-key') || event_id;
    ...
    // Persist to Blobs (append)
    try {
      const store = getStore({ name: 'ats' });
      const key = `${now.slice(0, 10)}.ndjson`;
      await store.append(key, JSON.stringify(payload) + '\n');
    } catch (e) {
    ...
    if (ATS_VENDOR === 'greenhouse') {
      ...
      try {
        ...
        try {
          ...
          const key = `${now.slice(0, 10)}.ndjson`;
          await dlq.append(key, JSON.stringify({ error: String(err), payload }) + '\n');
        } catch (e) {
    ...
    return new Response(JSON.stringify({ ok: true, event_id, idempotency_key }), {
      status: 200,
